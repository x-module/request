...
func TestRequestInterceptor(t *testing.T) {
	...
	req := NewRequest()
	req.UseRequest(func(ctx *context.Context, h context.Handler) {
		ctx.Request.Header.Set("Client", "request")

		ctx.Response.StatusCode = 201
		ctx.Response.Status = "201 Created"

		ctx.Response.Header.Set("Server", "request")
		utils.WriteBodyString(ctx.Response, "Hello, request")
	...
	st.Expect(t, res.RawRequest.Header.Get("Client"), "request")
	st.Expect(t, res.RawResponse.Header.Get("Server"), "request")
	st.Expect(t, res.String(), "Hello, request")
}
